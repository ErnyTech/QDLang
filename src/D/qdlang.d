...
private {
	...
	string generateFunQt(int n, string name, string nameAliasLib) {
		enum s = "QtE5";
		return "funQt("~to!string(n)~",true"~",qdlang"~",libraryName"~`,"`~name~`"`~",showError);";
	}
...
int LoadQt(bool showError) { ///  Загрузить DLL-ки Qt и QtE
    import core.runtime;
	string libraryName;

	version (Windows) {
        libraryName = "libQDLangCPP.dll";
	}
	
	version (linux) {
        libraryName = "libQDLangCPP.so";
	}
	
	version (OSX) {
		string[] libs = ["QtCore", "QtGui", "QtWidgets", "QtDBus" , "QtPrintSupport"];
		
		foreach(lib; libs) {
			void* h = Runtime.loadLibrary(lib);
		}
		
		libraryName = "libQDLangCPP.dylib";
	}

	void* qdlang = Runtime.loadLibrary(libraryName);
	
	if (!qdlang) {
        MessageErrorLoad(showError, libraryName);
        return 1;
	}
	...
	mixin(generateFunQt(	393, 	"QBitmap_delete1"					,"Widgets"));
	...
	if(showError) {    
        auto isFirstError = true;
        
        for(int i; i != maxValueInPFunQt; i++) {
            if(!pFunQt[i]) {
                if (isFirstError) {
...
template QDLangEntryPoint(alias mainFormName)
{
	import std.string : format;

	enum QDLangEntryPoint = format(
		`
			import core.runtime;
			import std.stdio;
...
extern "C" void* QImage_pixelColor(QImage* qi, int x, int y)  {
